...
| `STOCK_LIST_SOURCE` | 自选股来源：`env`(默认) 或 `wencai`(动态) | 可选 |
| `WENCAI_STOCK_QUERY` | 文财动态选股语句模板（默认：昨日连板天梯筛选） | 可选 |
| `WENCAI_COOKIE` | 文财查询 Cookie（某些环境下需要） | 可选 |
...
> 当 `STOCK_LIST_SOURCE=wencai` 时，系统会优先用文财动态生成股票列表；失败时自动回退到 `STOCK_LIST`。
